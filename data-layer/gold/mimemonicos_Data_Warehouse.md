# Arquivo de Mimemônicos e Dicionário de Dados (Camada Gold)

Este documento define os padrões de nomenclatura e o significado das colunas utilizadas no Data Warehouse (camada Gold) do projeto, conforme definido no DDL `create_table_gold.sql`.

## 1. Padrões de Nomenclatura (Mimemônicos)
...
| `fato_`        | Fato (Fact)          | Prefixo para tabelas fato. Elas contêm os eventos, métricas e valores numéricos.                         |
...
| `iata_code`    | IATA Code            | Sufixo para colunas que armazenam os códigos de negócio (ex: `LA` para LATAM, `JFK` para o aeroporto).    |

---

## 2. Dicionário de Tabelas (Star Schema)

Nosso Data Warehouse segue um **Star Schema**, composto por uma tabela Fato central (`fato_flights`) e três dimensões (`dim_airport`, `dim_airline`, `dim_date`).
